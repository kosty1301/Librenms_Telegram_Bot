...
class AlertDispach:
    def __init__(self, alert_list: list):
        self.recovered_query = (Device.select()
                                .where(Device.alert_is_send == 1)
                                .execute())

        self.device_alert_query = (Device.select()
                                   .where(Device
                                          .librenms_id
                                          .in_(tuple(alert_list))))

        self.device_is_recovered = [dev for dev in self.recovered_query
                                    if str(dev) not in alert_list]

        self.new_alerts = [dev for dev in self.device_alert_query
                           if dev.last_alert_data is None]

        self.alerts_to_send = [dev for dev in self.device_alert_query
                               if dev not in self.new_alerts and
                               dev.alert_is_send == 0]
            ...
            if delay_is_over(dev):
                for user in dev.device_user:
                    bot.send_message(f'{DEVICE_ALERT} \n {dev.name}'
                                     f'{dev.last_alert_data}', user.user)
